...
<template>
  <section class="page">
    <header class="page__header">
      <h1>时段管理</h1>
      <p class="muted">
        当前前端仅封装了只读查询 GET /specialists/:id/slots。若后端提供 POST/PATCH
        /specialist/slots 等写入接口，可在 src/api/client.js 增加方法后再接表单。
      </p>
    </header>
    ...
    <div class="card">
      ...
      <button type="button" class="btn" :disabled="loading" @click="load">查询时段</button>
    </div>

    <div v-if="error" class="banner banner--error" role="alert">{{ error }}</div>

    <div v-if="slots.length" class="card">
      <div class="title">结果（{{ slots.length }}）</div>
      <ul class="list">
