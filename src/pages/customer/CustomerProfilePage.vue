...
<template>
  <section class="page">
    <header class="page__header">
      <h1>个人资料</h1>
      <p class="muted">GET /me 与 PATCH /me</p>
    </header>
    ...
    <div class="card">
      ...
      <template v-else>
        ...
        <p v-if="auth.user?.email" class="muted small">邮箱：{{ auth.user.email }}（只读，视后端 PATCH 能力而定）</p>
        <p v-if="auth.user?.role" class="muted small">角色：{{ auth.user.role }}</p>
