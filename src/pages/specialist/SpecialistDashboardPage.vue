...
<template>
  <section class="page">
    <header class="page__header">
      <h1>专家仪表盘</h1>
      <p class="muted">待确认预约来自 GET /specialist/booking-requests</p>
    </header>

    <div v-if="error" class="banner banner--error" role="alert">{{ error }}</div>

    <div class="card">
      <div class="title">待处理（Pending）</div>
      <p v-if="loading" class="muted">加载中…</p>
      <p v-else class="stat">
        <span class="num">{{ pending.total ?? (pending.items || []).length }}</span>
        <span class="muted">条</span>
      </p>
      <RouterLink class="link" :to="{ name: 'specialist.requests' }">前往处理</RouterLink>
    </div>
